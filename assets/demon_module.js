  /* ===================== 악마 패턴 (무한질주 전용) =====================
     ...
     정답:
       ...
       까마귀   악마를 지난 직후 점프 (2단이 여유 있다)

     악마는 "그때 공중에 있으면 죽는다"는 장치다. 악마의 점프 높이는 시간이 아니라
     x 위치로만 계산되므로 스크롤 속도와 무관하게 언제나 같은 자리에서 같은 높이로 뜬다.
     즉 한 번 외우면 반드시 통과할 수 있고, 운에 맡기는 구간이 없다.

     [game.js 를 건드리지 않고 되는 이유]
     - 장애물 4개를 이 파일이 직접 만들어 obstacles 배열에 넣는다. game.js 의 스폰
       함수를 쓰지 않으므로 그쪽 동작(콤보 개 붙이기 등)에 휘둘리지 않는다.
     ...
     - 패턴이 흐르는 동안에는 crowPending 을 켜서 일반 장애물 스폰을 멈춘다 (까마귀
       경고가 쓰는 것과 같은 장치다).
     - 까마귀 뒤에는 빈 구간을 강제로 둔다. 까마귀를 2단으로 넘은 사람이 착지할 자리가
       없으면 안 되기 때문이다.

     숫자 조절은 assets/endless_tune.js 의 8) 항목에서 한다.
     ==================================================================== */
  var DEMON_ON       = !!window.ENDLESS_MODE && (window.ENDLESS_DEMON !== false);
  var DEMON_GAP1     = window.ENDLESS_DEMON_GAP1 || 0.86;   // 똥1 -> 똥2
  var DEMON_GAP2     = window.ENDLESS_DEMON_GAP2 || 0.78;   // 똥2 -> 악마
  var DEMON_GAP3     = window.ENDLESS_DEMON_GAP3 || 0.90;   // 악마 -> 까마귀
  var DEMON_TAIL     = window.ENDLESS_DEMON_TAIL || 1.20;   // 까마귀 뒤 빈 구간
  ...
  var demonNextAt = 0;        // 이 게임시간이 지나면 패턴을 낸다
  var demonPieces = null;     // 패턴이 흐르는 동안의 장애물 4개
  var demonBody   = null;     // 그중 악마
  ...
  function demonMakeBody(x, appW, appH){
    var dh = DOG_H * DEMON_H_MULT;
    var dw = dh * 1.018;                       // boss.webp 원본 비율에 가깝다
    var el = document.createElement('div');
    // 'boss' 클래스는 그림 스타일(테두리 없음 + 가만히 들썩이는 연출)을 물려받기 위한
    // 것이고, 판정/처치 로직은 type 으로 갈리므로 보스전과 전혀 섞이지 않는다.
  ...
  function demonSpawn(){
    ...
    var p2 = demonMakePoop(x0 + DEMON_GAP1 * appH);
    demonBody = demonMakeBody(x0 + (DEMON_GAP1 + DEMON_GAP2) * appH, appW, appH);
    var cr = demonMakeCrow(x0 + (DEMON_GAP1 + DEMON_GAP2 + DEMON_GAP3) * appH);
  ...
  function demonRelease(){
    crowPending = false;
    spawnTimer = 0;
    // 까마귀를 2단으로 넘은 사람이 착지할 시간을 벌어준다. 새 장애물은 화면 오른쪽
    // 밖에서 생겨 1초쯤 걸려 다가오므로, 여기에 한 칸 더 얹으면 충분하다.
    nextSpawnIn = Math.max(700, (OBST_GAP_PX / Math.max(1, speed)) * 1000);
    demonPieces = null;
    demonBody = null;
    demonNextAt = gameTime + demonPickWait();
  }
  ...
  function demonUpdate(dt){
    ...
    if (demonPieces){
      ...
      // 패턴의 마지막 조각(까마귀)이 캐릭터를 충분히 지나가면 일반 스폰을 다시 켠다.
      var cr = demonPieces[3];
      var gone = (obstacles.indexOf(cr) < 0) || (cr.x < charLeftPx - DEMON_TAIL * appH);
    ...
    // 까마귀 경고가 떠 있거나 엔딩 중이면 다음 기회로 넘긴다.
    if (crowPending) return;
    if (gameTime < demonNextAt) return;
    // 화면 오른쪽이 비어 있을 때만 시작한다. 방금 생긴 일반 장애물이 오른쪽에 남아
    // 있으면 패턴의 똥1과 겹쳐서 피할 수 없는 배치가 만들어진다. 한 칸(OBST_GAP_PX)
    // 이상 비었을 때까지 기다린다 - 조건이 맞을 때까지 미루는 것뿐이므로 안전하다.
    var clearX = appW - OBST_GAP_PX;
